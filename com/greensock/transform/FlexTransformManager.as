...
package com.greensock.transform {
	...
	[Event(name="tmSeizeCursor", type="com.greensock.events.TransformEvent")]
	[Event(name="tmReleaseCursor", type="com.greensock.events.TransformEvent")]
	...
	[Event(name="tmStartInteractiveMove", type="com.greensock.events.TransformEvent")]
	[Event(name="tmStartInteractiveScale", type="com.greensock.events.TransformEvent")]
	[Event(name="tmStartInteractiveRotate", type="com.greensock.events.TransformEvent")]
 ...
 * basically a wrapper that makes it compatible with Flex.
 *  
 ...
 * 		<li> FlexTransformManager's width and height should be fixed values, not percentages.</li>
 ...
 * 
 * <p><b>EXAMPLES:</b></p>
 * <p>To make two Images (myImage1 and myImage2) transformable using the default settings:</p>
 * 
<listing version="3.0">
 ...
 * 
 * 
 * <p>To make the two Images transformable, constrain their scaling to be proportional (even if the user is not holding
 * down the shift key), call the onScale function everytime one of the objects is scaled, lock the rotation value of each 
 * Image (preventing rotation), and allow the delete key to appear to delete the selected Image from the stage:</p>
 * 
 * <listing version="3.0">
 	&amp;lt;?xml version="1.0" encoding="utf-8"?&amp;gt;
 ...
 * 
 * <p><b>Copyright 2008-2012, GreenSock. All rights reserved.</b> This work is subject to the terms in <a href="http://www.greensock.com/eula.html">http://www.greensock.com/eula.html</a> or for corporate Club GreenSock members, the software agreement that was issued with the corporate membership.</p>
 * 
	...
	public class FlexTransformManager extends Canvas {
		...
		public function FlexTransformManager($vars:Object = null) {
			...
			if (!_initted) {
				...
				addNonStandardScalingClass("spark.components.TextArea", TransformManager.SCALE_WIDTH_AND_HEIGHT, true);
				addNonStandardScalingClass("mx.controls.TextInput", TransformManager.SCALE_WIDTH_AND_HEIGHT, true);
				addNonStandardScalingClass("spark.components.TextInput", TransformManager.SCALE_WIDTH_AND_HEIGHT, true);
				...
				addNonStandardScalingClass("spark.components.RichEditableText", TransformManager.SCALE_WIDTH_AND_HEIGHT, true);
				addNonStandardScalingClass("mx.controls.Label", TransformManager.SCALE_WIDTH_AND_HEIGHT, false);
				addNonStandardScalingClass("spark.components.Label", TransformManager.SCALE_WIDTH_AND_HEIGHT, false);
				
				_initted = true;
			}
			_manager = new TransformManager($vars);
			trace("initial bounds width: "+this.width+", height: "+this.height);
		...
		protected function onAddToStage($e:Event):void {
			...
			if (_manager.bounds != null && (_manager.bounds.width == 0 || _manager.bounds.height == 0)) {
				_manager.bounds = (this.parent != null) ? new Rectangle(0, 0, this.width || this.parent.width, this.height || this.parent.height) : new Rectangle(0, 0, this.width, this.height);
			}
		...
		/** @private **/
		private function _sparkSafeRemove(target:DisplayObject):void {
			if (target != null && target.parent != null && target.parent != this && target.parent.hasOwnProperty("removeElement")) {
				try {
					Object(target.parent).removeElement(target); //if we don't do this first and the parent is a spark component, it will throw errors.
				} catch (error:Error) {
		...
		public function addItems($targetObjects:Array, $scaleMode:String="scaleNormal", $hasSelectableText:Boolean=false):Array	{
			for (var i:int = 0; i < $targetObjects.length; i++) {
				if ($targetObjects[i].parent != this) {
					_sparkSafeRemove($targetObjects[i]);
		...
		override public function addEventListener($type:String, $listener:Function, $useCapture:Boolean=false, $priority:int=0, $useWeakReference:Boolean=false):void {
			...
			switch ($type) {
				...
				case TransformEvent.SEIZE_CURSOR:
				case TransformEvent.RELEASE_CURSOR:
				case TransformEvent.SELECTION_CHANGE:
				case TransformEvent.START_INTERACTIVE_MOVE:
				case TransformEvent.START_INTERACTIVE_SCALE:
				case TransformEvent.START_INTERACTIVE_ROTATE:
		 ...
		 * "topLeft", "top", "topRight", "right", "bottomRight", "bottom", "bottomLeft", or "left" and you can 
		 ...
		 * @param alignment "topLeft", "top", "topRight", "right", "bottomRight", "bottom", "bottomLeft", or "left"
		...
		public function addSelectionBoxElement(element:DisplayObject, alignment:String="topRight", xOffset:Number=0, yOffset:Number=0, underHandles:Boolean=false):void {
			_manager.addSelectionBoxElement(element, alignment, xOffset, yOffset, underHandles);
		}
		...
		public function removeSelectionBoxElement(element:DisplayObject):void {
			_manager.removeSelectionBoxElement(element);
		}
...
&amp;lt;settings allowDelete="1" allowMultiSelect="1" autoDeselect="1" constrainScale="0" lockScale="1" scaleFromCenter="0" lockRotation="0" lockPosition="0" arrowKeysMove="0" forceSelectionToFront="1" lineColor="3381759" handleColor="16777215" handleSize="8" paddingForRotation="12" hideCenterHandle="0"/&amp;gt;
		...
		override public function setActualSize(w:Number, h:Number):void {
			var r:Rectangle = _manager.bounds;
			if (h != 0 && w != 0 && r != null) {
				r = r.clone();
				if (r.width == super.width || super.width == 0) {
					r.width = w;
				}
				if (r.height == super.height || super.height == 0) {
					r.height = h;
				}
				//_manager.bounds = r;
			}
...
//---- GETTERS / SETTERS -----------------------------------------------------------------------------------------------------
		...
		override public function set width($n:Number):void {
			var r:Rectangle = _manager.bounds;
			if (r != null && this.height != 0 && (r.width == super.width || super.width == 0)) {
				r = r.clone();
				r.width = $n;
				_manager.bounds = r;
			}
		...
		override public function set height($n:Number):void {
			var r:Rectangle = _manager.bounds;
			if (r != null && this.width != 0 && (r.height == super.height || super.height == 0)) {
				r = r.clone();
				r.height = $n;
				_manager.bounds = r;
			}
		...
		public function get lineThickness():Number {
			return _manager.lineThickness;
		}
		public function set lineThickness($value:Number):void {
			_manager.lineThickness = $value;
		}
