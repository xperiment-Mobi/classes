 ...
 * to interactively crop a DisplayObject using a rectangular mask. A FlexCrop object is a <code>UIComponent</code> that places 
 * itself in front of its <code>target</code> (the DisplayObject that it should crop) and it also creates another UIComponent object 
 * that serves as the target's <code>mask</code>. So the FlexCrop object and the mask UIComponent are both automatically added to the
 * display list. When the FlexCrop's <code>attached</code> property is <code>true</code>, the <code>target</code> will 
 * move/scale/rotate along with the FlexCrop object (changes to the FlexCrop are also applied to the target). 
 * When <code>maskEnabled</code> is <code>true</code>, the <code>target</code> will be masked visually. 
 * 
 * <p>The FlexCrop object is treated as just another TransformItem in the associated TransformManager so that it is able 
 * to be moved/scaled/rotated/selected. </p>
 * 
 * <p>When the FlexCrop's <code>cropMode</code> property is <code>true</code>, it automatically detaches from the target
 ...
 * 		<li>The <code>alpha</code> of the FlexCrop is set to 0.3 so that the rectangle becomes visible.</li>
 ...
 * 
 * <p>To enter cropMode, simply double-click on the FlexCrop object or manually set <code>cropMode</code> to <code>true</code>.
 ...
 * method to add a cropping button/icon to the actual selection box if you prefer. To exit cropMode, double-click the FlexCrop object 
 * again or deselect both the target and FlexCrop object (by clicking off of them) (you can disable this behavior with the 
 ...
 */	
	public class FlexCrop extends UIComponent {
		/** @private **/
		 ...
		 * Constructor
		 * 
		 * @param target the target DisplayObject that the FlexCrop object should affect (mask/crop). 
		 * @param manager the TransformManager instance that should be used to manage the transformations/selections
		 * @param attached if <code>true</code> (the default), transformations that are made to the FlexCrop (move/scale/rotation) will also affect to the <code>target</code>. Set <code>attached</code> to <code>false</code> to move/scale/rotate them independently.
		 * @param fillColor the fill color (hex value) of the FlexCrop object. When <code>cropMode</code> is <code>false</code> the FlexCrop's alpha is 0, so you won't see the fill color unless <code>cropMode</code> is <code>true</code>. This basically controls the interactive mask color.
		 * @param alpha the initial alpha value of the FlexCrop object (typically 0 is best).
		 */
		...
		/**
		 * Adjusts the depth levels of the <code>target</code> or FlexCrop object in the display list so 
		 * that the FlexCrop is on top of the <code>target</code>.
		 * 
		 ...
		 * @param leaveTarget if <code>true</code>, the <code>target</code> will remain exactly where it is in the display list and the FlexCrop object will be moved above it rather than leaving the FlexCrop where it is and moving the <code>target</code> under it. 
		 */
		public function calibrateDepths(event:Event=null, leaveTarget:Boolean=false):void {
			if (_target.parent == null) {
				return;
			}
			var useElement:Boolean = _target.parent.hasOwnProperty("getElementIndex");
			var targetIndex:int = (useElement) ? Object(_target.parent).getElementIndex(_target) : _target.parent.getChildIndex(_target);
			if (this.parent != _target.parent) {
				if (useElement) {
					Object(_target.parent).addElementAt(this, targetIndex+1);
					Object(_target.parent).addElementAt(_mask, targetIndex+1);
				} else {
			...
			var thisIndex:int = (useElement) ? Object(this.parent).getElementIndex(this) : this.parent.getChildIndex(this);
			if (!leaveTarget && thisIndex == 0) {
				if (useElement) {
					Object(this.parent).setElementIndex(_target, 0);
					Object(this.parent).setElementIndex(this, 1);
				} else {
				...
				if (useElement) {
					Object(this.parent).setElementIndex(this, targetIndex + 1);
				} else {
					this.parent.setChildIndex(this, targetIndex + 1);
				}
			} else if (useElement) {
				Object(this.parent).setElementIndex(_target, thisIndex - 1);
			} else {
		...
		protected function _removeMasks(event:Event):void {
			if (this.parent != null) {
				if (_target.parent.hasOwnProperty("removeElement")) {
			...
			if (_mask.parent != null) {
				if (_mask.parent.hasOwnProperty("removeElement")) {
					Object(_mask.parent).removeElement(_mask);
				} else {
		...
		/** @private redraws the box inside the FlexCrop and _mask so that it matches the _target's native bounding box **/
		 ...
		 * @param alpha sets the <code>alpha</code> that the FlexCrop object should be when in <code>cropMode</code>. Set the <code>fillColor</code> to control the color.
		 ...
		 * @param autoApplyOnDeselect if <code>true</code>, deselecting the FlexCrop object and its <code>target</code> will automatically set <code>cropMode</code> to <code>false</code>.
		 */
		...
		/**
		 * Forces an update of the FlexCrop object so that it either applies any recent scale/rotation/movement to the <code>target</code>
		 ...
		 * @param calibrateBox if <code>true</code>, the FlexCrop's rectangle will be updated to match the native bounding box of the <code>target</code>. This can be helpful if the <code>target</code>'s size or bounding box changed since the last update.
		 */
		...
		protected function _update(event:Event, force:Boolean=false):void {
			...
			if (_attached || force) {
				...
				/* If you want to enable a scaleMode like scaleWidthAndHeight, you can tap into this but the _target's rotation and the FlexCrop's/mask's rotation MUST be the same when attached otherwise skewing would happen and setting the width/height wouldn't give accurate results (it's impossible)
		...
		protected function _deleteHandler(event:TransformEvent):void {
			if (_targetTransformItem) {
				...
				if (_target.parent.hasOwnProperty("removeElement")) {
					Object(_target.parent).removeElement(_target);
				} else {
...
//---- GETTERS / SETTERS ------------------------------------------------------------------------------
		...
		/** If <code>true</code>, transformations that are made to the FlexCrop (move/scale/rotation) will also affect to the <code>target</code>. Set <code>attached</code> to <code>false</code> to move/scale/rotate the crop and its target independently. **/
		...
		/** If <code>true</code>, the <code>target</code> will be masked by the crop. Technically the target's <code>mask</code> gets set to a UIComponent object that the FlexCrop creates internally. **/
		...
		/**  When the FlexCrop's <code>cropMode</code> property is <code>true</code>, it automatically detaches from the target
		 * (<code>attached</code> is set to <code>false</code>) and the following also occur, which you can customize 
