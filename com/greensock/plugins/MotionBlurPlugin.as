...
/**
 * [AS3/AS2 only] MotionBlurPlugin provides an easy way to apply a directional blur to a DisplayObject based on its velocity
 ...
 * 
 * <p>motionBlur recognizes the following properties:</p>
 * <ul>
 ...
 * </ul>
 * 
 * <p>You can optionally set motionBlur to the Boolean value of <code>true</code> in order to use the defaults. (see below for examples)</p>
 * 
 * <p>Also note that due to a bug in Flash, if you apply motionBlur to an object that was masked in the Flash IDE it won't work
 ...
 * like <code>z, rotationY, rotationX,</code> or <code>rotationZ</code>.</p>
 * 
 * <p><b>USAGE:</b></p>
 * <listing version="3.0">
 ...
 * 
 * <p>MotionBlurPlugin is a <a href="http://www.greensock.com/club/">Club GreenSock</a> membership benefit. 
	...
	public class MotionBlurPlugin extends TweenPlugin {
		...
		override public function _onInitTween(target:Object, value:*, tween:TweenLite):Boolean {
			...
			if (_tween.vars.x != null || _tween.vars.y != null) {
				var x:Number = (_tween.vars.x == null) ? _target.x : (typeof(_tween.vars.x) == "number") ? _tween.vars.x : Number(_tween.vars.x.split("=").join("")) + _target.x;
				var y:Number = (_tween.vars.y == null) ? _target.y : (typeof(_tween.vars.y) == "number") ? _tween.vars.y : Number(_tween.vars.y.split("=").join("")) + _target.y;
			...
			if (typeof(value) == "object" && value.containerClass is Class) {
				_sprite = new (value.containerClass as Class)();
			} else if (_isFlex) {
		...
		public function _onDisable():Boolean {
			if (_tween._time != _tween._duration && _tween._time != 0) { //if the tween is on a TimelineLite/Max that eventually completes, another tween might have affected the target's alpha in which case we don't want to mess with it - only disable() if it's mid-tween. Also remember that from() tweens will complete at a value of 0, not 1.
				var wasActive:Boolean = (_sprite.parent != null);
				_disable();
				return wasActive;
			}
			return false;
		...
		override public function setRatio(v:Number):void {
			...
			var dx:Number = _target.x - _xCurrent,
				dy:Number = _target.y - _yCurrent,
				rx:Number = _target.x - _xRef,
				ry:Number = _target.y - _yRef;
