...
public class ShuffleSuperUnique
{
	...
	public static function DO(info:String, list:XMLList):void {
		...
		if (arr.length == 0 || arr.length > 2) err('you must define ONE or TWO variables (these are defined:' + arr + ')');
		str2 = arr.pop();
		if (arr.length > 0)    str1 = arr.pop();
		else str1 = '';

		if(list.@[str2].toString().length == 0) {
			err('One/both of the properties to modify (' + str2 + ') is not present.');
		}

		if(str1!='' && list.@[str1].toString().length == 0) {
			err('One/both of the properties to modify (' + str1 + ') is not present.');
		}

		var result:Object = process(getParams(pattern), list.@[str1], list.@[str2]);

		if (result.arr1)    list.@[str1] = result.arr1.join(split);
		if (result.arr2)    list.@[str2] = result.arr2.join(split);
		//trace(list.@[str2])
		//}
	...
	{
		var split1:Array, split2:Array;
		if(str1!='')	split1 = getSplit(params,str1.split(";"));
		split2 = getSplit(params,str2.split(";"));

		if(str1!='')	uniqueShuffle(split1,split2,'ident');
		else 			codeRecycleFunctions.arrayShuffle(split2);

		if(str1!='')randomlySelect(split1);
	...
	private static function getType(str:String, params:Object):Object
	{
		...
		obj.ident= str.substr(0,params.start) + str.substr(params.start+params.num);
		obj.name = str;
		return obj;
	}
